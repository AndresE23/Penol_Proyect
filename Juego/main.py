...
class TicTacToe:
    ...
    def new_game(self, event):
        ...
        self.set_status(f'{self.current_player} jugador...')

    def next_turn(self):
        winner = self.check_winner()
        if winner == "tie":
            self.set_status("JajajajaQUE BOBOS PARCE!")
            self.current_player = "" # i.e., game ended
            return
        elif winner is not None:
            self.set_status(f'{winner} Aysss ganador')
            self.current_player = "" # i.e., game ended
            return
    ...
    def check_winner(self):
        ...
        for combo in self.winning_combos:
            ...
            if winner:
                ...
                for i, j in combo:
                    self.cells[i][j].add_class("Aysssganador")
                return winner
